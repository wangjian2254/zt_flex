package uicontrol
{
	...
	include "../metadatacu/FocusStyles.as"
	include "../metadatacu/IconColorStyles.as"
	include "../metadatacu/LeadingStyle.as"
	include "../metadatacu/PaddingStyles.as"
	include "../metadatacu/SkinStyles.as"
	include "../metadatacu/TextStyles.as"
	 ...
	 *  @productversion Flex 3
	 */
	public class DDBHFliterComboBox extends ComboBase
		implements IDataRenderer, IDropInListItemRenderer,
		IListItemRenderer
	{
		include "../metadatacu/Version.as";
		 ...
		 *  @productversion Flex 3
		 */
		public function DDBHFliterComboBox()
		{
			super();
		...
		private var _valueField:String;//获取值如果没有选中值则返回textinput中的值
		
		private var _filtervalue:int=0;
		public function get filtervalue():int
		{
			return _filtervalue;
		}
...
//物料过滤值
		...
		//			return selectValue();
		//		}
		public function selectValue():Object{
			if(valueField==null){
				return _data;
			}
			var item:Object;
			if(selectedIndex==-1){
				...
				item=dataProvider.getItemAt(selectedIndex);
				return item[valueField];
			}
		}
		...
		public function setValue(value:String):void{
			if(valueField&& dataProvider is ArrayCollection&&dataProvider!=null){
				var found:Boolean = false;
//				var listCursor:IViewCursor = collection.createCursor();
				var i:int = 0;
				var obj:Object;
				for(;i<dataProvider.length;i++){
					obj=dataProvider.get(i);
...
//				do
//				{
//					if (listCursor.current!=null&&value == listCursor.current[valueField])
//					{
//						selectedIndex = i;
//						selectionChanged = true;
//						found = true;
...
//				while (listCursor.moveNext());
				...
				{
					selectedIndex = -1;
					textInput.text=new String(value);
					prompt=new String(value);
				}
			}
		}
		...
		private function searchKeyWord(keyWord:String):void{  
			...
			if(dp.length>0){  
				...
				//				textInput.selectRange(0,keyWord.length);
				open();  
			}else{  
				dataProvider=ac;  
				//				selectedIndex=-1;  
		...
		override public function set selectedIndex(value:int):void
		{
			...
			else if (textInput && prompt && textInput.text=="")
		...
		{
			if (DDBHFliterComboBox.createAccessibilityImplementation != null)
				DDBHFliterComboBox.createAccessibilityImplementation(this);
		}
													  ...
													  unscaledHeight:Number):void
		{
			...
			if (labelFieldChanged)
			{
				...
				selectionChanged = true;
				if (!explicitText && selectedIndex != -1) 
					textInput.text = selectedLabel;
			...
			if (selectionChanged)
			{
				if (!textChanged)
				{
					if (selectedIndex == -1 && prompt && textInput.text=="")
						textInput.text = prompt;
					else if (!explicitText && selectedIndex != -1)
						textInput.text = selectedLabel;
				}
		...
		override protected function commitProperties():void
		{
			...
			if (promptChanged && prompt != null && selectedIndex == -1&& textInput.text=="")
			{
				promptChanged = false;
				textInput.text = prompt;
			}
		...
		private function getDropdown():ListBase
		{
			...
			var m:Matrix = MatrixUtil.getConcatenatedMatrix(this,null);
